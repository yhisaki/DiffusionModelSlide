...
#slide(title: [サンプリング: スコアを用いたランジュバン・ダイナミクス @ncsn])[

  スコア関数 $s_theta (x) approx nabla_x log q(x)$ を手に入れた後、どのようにサンプルを生成するか？
  #v(0.5em)

  - *ランジュバン・ダイナミクス (Langevin Dynamics):*
...
#slide(title: "なぜ単純なスコアマッチングは失敗するのか")[
  ...
  3. *ランジュバン・ダイナミクスの遅い混合 (Slow Mixing)*
...
#slide(title: "課題2: 確率フローによるサンプリング")[
  ...
  - *サンプリングのメカニズム:*
    1. サンプル $x_T tilde p_T$ (単純なガウス分布、どこでもサポートあり)。
...
#slide(title: "ロードマップ: モデルの構築")[
  ...
  1. *ステップ 1: SDEの特定 (VP SDE)*
    ガウスノイズへの収束を保証するために、特定のドリフト $f(x, t)$ と拡散 $g(t)$ を定義する。
  2. *ステップ 2: スコアの学習 (スコアマッチング)*
...
#slide(title: "VP SDE: 標準的な確率フロー")[
  ...
  - *時間発展の可視化 ($x_0 = 1$):*
    ...
    $x_0 = 1$ から始まる単一の1次元データポイントを時間 $t in [0, 1]$ にわたって追跡してみよう。
  #v(0.5em)
  ...
  - *ドリフト (平均):*
    ...
    元の信号は体系的に消去される。
  - *拡散 (分散):*
    $plus.minus 2 sigma(t)$ の区間(ここで $sigma(t) = sqrt(1 - alpha_t)$)は、$t=1$ で分布が標準ガウスノイズ $cal(N)(0, 1)$ に完全に一致するまで着実に拡大する。
]

#title-slide[
  ステップ 2: スコアの学習 (スコアマッチング)
]
...
#slide(title: "学習: 連続時間スコアマッチング")[
  ...
  $arrow.double$ この単一の目的関数を最適化することで、$s_theta (x, t)$ は連続ベクトル場を学習する。
  その後、これをReverse ODEに直接代入してデータを生成できる！
]
...
#slide(title: "理論: 確率フローODE")[
  ...
  データを生成するには、拡散過程を逆転させる必要がある。
  確率過程 (SDE) と決定論的過程 (ODE) を結びつける基本定理を利用する。
...
#slide(title: "まとめ: 拡散フレームワーク")[
  ...
  3. *生成 (Reverse ODE):*
    学習したスコアを使用してノイズ $p_T$ からデータ多様体 $p_0$ へナビゲートし、決定論的ODEを逆方向に解くことで新しいデータを生成した。
]
...
#slide(title: "Conditional Generation: Bayes' Rule")[
  ...
  - *メカニズム:*
    ...
    2. *Guidance項:* 二次的なベクトル場として機能し、軌跡を条件 $y$ を満たすデータ多様体の領域へと能動的に押しやる。
]
...
#slide(title: "応用1: 潜在空間補間")[
  ...
  - *補間の方法: 画像の補間の例*
    ...
    2. *補間:* 球面事前分布空間でパラメータ $lambda in [0, 1]$ を使用して潜在変数をブレンドする(例:  $z_lambda = "Slerp"(x_T^((A)), x_T^((B)), lambda)$)。
